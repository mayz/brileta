...
Do NOT run individual tools (pytest, ty, ruff, clang-format, clang-tidy) separately first. Just run `make` once - it handles formatting (Python and C), linting, type checking, and tests together. See the `Makefile` for details.
