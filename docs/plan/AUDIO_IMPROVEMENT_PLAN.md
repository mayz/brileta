...
### 2. Fix Sound Propagation Physics
**Problem:** Current inverse square law implementation at line 266 is physically incorrect and doesn't match how sound actually propagates in games.

**Current Code:** `catley/sound/system.py:264-267`
```python
volume_factor = 1.0 / (1.0 + falloff_distance**2)
```

**Solution: Industry-Standard Rolloff Model**

**Why this approach:**
- **Battle-tested** in Unity, Unreal, FMOD, and Wwise - proven to work well in thousands of games
- **Designer-friendly** - easy to understand and tune with just 2-3 intuitive parameters
- **Performant** - simple division operation, no expensive logarithms
- **Flexible** - different rolloff factors for different sound types (explosions vs ambience)
- **Industry compatible** - audio designers will immediately understand these parameters

**Implementation:**

**Step 1:** Add rolloff_factor to SoundDefinition dataclass (`catley/sound/definitions.py:30`):
```python
@dataclass
class SoundDefinition:
    ...
    falloff_start: float = 2.0
    max_distance: float = 10.0
    priority: int = 5
    rolloff_factor: float = 1.0  # ADD THIS: How quickly sound attenuates
```

**Step 2:** Update _calculate_volume method (`catley/sound/system.py:264-267`):
```python
# Replace the current inverse square calculation with:
if falloff_distance <= 0:
    volume_factor = 1.0
else:
    # Industry-standard distance attenuation model
    reference_distance = sound_def.falloff_start
    rolloff_factor = sound_def.rolloff_factor
    volume_factor = reference_distance / (reference_distance + rolloff_factor * falloff_distance)
```

**Step 3:** Update existing sound definitions with appropriate rolloff factors:
```python
SOUND_DEFINITIONS = {
    ...
    "waterfall_ambient": SoundDefinition(
        # ... existing fields ...
        rolloff_factor=0.6,  # Even gentler for water
    ),
...
**Recommended rolloff factors by sound type:**
- UI sounds: 0.0 (no distance attenuation)
- Ambient loops (fire, water): 0.5-0.8 (gentle falloff)
- Footsteps, impacts: 1.0 (standard inverse distance)
- Explosions, loud one-shots: 1.5-2.0 (sharp falloff)
