...
## ✅ **RESOLVED: Actor "Buzzing" Visual Artifact**

**Issue:** Actors exhibited a visual "buzzing" or flickering effect while stationary, caused by transparency fluctuations.

**Root Cause:** The `alpha` parameter (meant for position interpolation) was incorrectly used as the color alpha channel in `draw_actor_smooth()`, causing actors to flicker between transparent and opaque as interpolation values changed each frame.

**Fix Applied:**
1. **Code Fix:** Changed color alpha from `alpha` to `1.0` (fully opaque) in `catley/backends/moderngl/graphics.py`
2. **Parameter Renaming:** Renamed `alpha` → `interpolation_alpha` throughout codebase for clarity
3. **Regression Test:** Added `test_draw_actor_smooth_uses_opaque_color_alpha()` to prevent future regressions
4. **Documentation:** Added clear comments explaining interpolation vs color alpha distinction

**Status:** ✅ **COMPLETELY RESOLVED** - No more actor buzzing, multi-layer protection against regression implemented.
