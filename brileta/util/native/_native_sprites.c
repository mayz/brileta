 ...
 */
static int
get_canvas_buffer_ex(PyObject *obj, Py_buffer *buf, int *h, int *w, uint8_t **data, int writable) {
    ...
    if (buf->ndim != 3 || buf->shape[2] != 4 || buf->itemsize != 1) {
        ...
        PyErr_SetString(PyExc_TypeError, "canvas must be a contiguous (H, W, 4) uint8 array");
        return -1;
    }
...
static int get_canvas_buffer(PyObject *obj, Py_buffer *buf, int *h, int *w, uint8_t **data) {
    return get_canvas_buffer_ex(obj, buf, h, w, data, /*writable=*/1);
}

static int
get_readonly_canvas_buffer(PyObject *obj, Py_buffer *buf, int *h, int *w, uint8_t **data) {
...
static inline void
c_alpha_blend(uint8_t *data, int h, int w, int x, int y, int r, int g, int b, int a) {
    if (x < 0 || x >= w || y < 0 || y >= h)
        return;
    if (a <= 0)
        return;
 ...
 */
static void c_composite_over(uint8_t *data,
                             int canvas_w,
                             int y_min,
                             int y_max,
                             int x_min,
    ...
    for (int row = y_min; row <= y_max; row++) {
        for (int col = x_min; col <= x_max; col++) {
            ...
            px[0] = (uint8_t)clampi((int)((sr * sa + px[0] * da * inv_src) * inv_out), 0, 255);
            px[1] = (uint8_t)clampi((int)((sg * sa + px[1] * da * inv_src) * inv_out), 0, 255);
            px[2] = (uint8_t)clampi((int)((sb * sa + px[2] * da * inv_src) * inv_out), 0, 255);
...
PyObject *brileta_native_sprite_composite_over(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "OiiiiOiii", &canvas_obj, &y_min, &y_max, &x_min, &x_max, &alpha_obj, &r, &g, &b))
        return NULL;
    ...
    Py_BEGIN_ALLOW_THREADS c_composite_over(
        data, w, y_min, y_max, x_min, x_max, src_alpha, r, g, b);
...
static void c_draw_line(
    uint8_t *data, int h, int w, int x0, int y0, int x1, int y1, int r, int g, int b, int a) {
    int dx = abs(x1 - x0);
    int dy = abs(y1 - y0);
    int sx = x0 < x1 ? 1 : -1;
    int sy = y0 < y1 ? 1 : -1;
...
PyObject *brileta_native_sprite_draw_line(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "Oddddiiii", &canvas_obj, &fx0, &fy0, &fx1, &fy1, &r, &g, &b, &a))
        return NULL;
...
PyObject *brileta_native_sprite_draw_thick_line(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "Oddddiiiii", &canvas_obj, &fx0, &fy0, &fx1, &fy1, &r, &g, &b, &a, &thickness))
        return NULL;
...
PyObject *brileta_native_sprite_draw_tapered_trunk(PyObject *self, PyObject *args) {
     ...
     * c_composite_over for each row to match Python's vectorized path. */
    {
        ...
        float *src_a = (float *)calloc((size_t)n_rows * (size_t)n_cols, sizeof(float));
        if (!src_a) {
            free(half_w);
            oom = 1;
            goto trunk_done;
 ...
 */
static inline float ellipse_alpha(float dist,
 ...
 */
static void batch_stamp_from_specs(uint8_t *data,
                                   int canvas_h,
                                   int canvas_w,
                                   const EllipseSpec *specs,
                                   int n_specs,
                                   int r,
                                   int g,
                                   int b,
    ...
    for (int i = 0; i < n_specs; i++) {
        if (specs[i].rx <= 0.0f || specs[i].ry <= 0.0f)
            continue;
...
PyObject *brileta_native_sprite_stamp_fuzzy_circle(PyObject *self, PyObject *args) {
    ...
    for (int row = y_min; row <= y_max; row++) {
        for (int col = x_min; col <= x_max; col++) {
            ...
            float inv_src = 1.0f - sa;
            float inv_out = 1.0f / out_a;
            px[0] = (uint8_t)clampi((int)((r * sa + px[0] * da * inv_src) * inv_out), 0, 255);
            px[1] = (uint8_t)clampi((int)((g * sa + px[1] * da * inv_src) * inv_out), 0, 255);
            px[2] = (uint8_t)clampi((int)((b * sa + px[2] * da * inv_src) * inv_out), 0, 255);
            px[3] = (uint8_t)clampi((int)(out_a * 255.0f), 0, 255);
        }
    }
...
PyObject *brileta_native_sprite_stamp_ellipse(PyObject *self, PyObject *args) {
    ...
    for (int row = y_min; row <= y_max; row++) {
        for (int col = x_min; col <= x_max; col++) {
            ...
            float sa = ellipse_alpha(dist, inner_fraction, outer_fraction, ef, edge_limit, opacity);
            if (sa <= 0.0f)
                continue;
...
PyObject *brileta_native_sprite_batch_stamp_ellipses(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "OOiiiidd", &canvas_obj, &ellipse_list, &r, &g, &b, &a, &falloff, &hardness))
        return NULL;
 ...
 */
static int parse_point_list(PyObject *list, int *n_out, float **x_out, float **y_out) {
    Py_ssize_t n = PyList_Size(list);
    if (n <= 0) {
        *n_out = 0;
        *x_out = NULL;
...
static inline void
append_ellipse_spec(EllipseSpec *specs, int *count, float cx, float cy, float rx, float ry) {
    assert(*count < MAX_CANOPY_ELLIPSES &&
           "canopy ellipse array overflow - increase MAX_CANOPY_ELLIPSES");
    if (*count >= MAX_CANOPY_ELLIPSES)
        return;
    if (rx <= 0.0f || ry <= 0.0f)
        return;
    specs[*count].cx = cx;
...
PyObject *brileta_native_sprite_generate_deciduous_canopy(PyObject *self, PyObject *args) {
    ...
    for (int i = 0; i < n_lobes; i++) {
        double angle =
            lobe_angle_offset + base_angle_step * (double)i + rng_uniform(&rng, -0.4, 0.4);
    ...
    for (int i = 0; i < n_central_fills; i++) {
        ...
        append_ellipse_spec(central_fills,
                            &n_central,
    ...
    int n_tip_shadow = select_unique_indices(&rng, n_tips, rng_int(&rng, 1, 3), selected_tip_idx);
    for (int i = 0; i < n_tip_shadow; i++) {
        int tip_idx = selected_tip_idx[i];
        float radius = (float)(base_radius * rng_uniform(&rng, 0.50, 0.68));
        append_ellipse_spec(shadows,
                            &n_shadows,
                            tip_xs[tip_idx] + (float)canopy_center_x_offset +
    ...
    int n_tip_mid = select_unique_indices(&rng, n_tips, rng_int(&rng, 1, 3), selected_tip_idx);
    ...
    for (int i = 0; i < n_lobes; i++) {
        ...
        for (int j = 0; j < n_for_lobe; j++) {
            ...
            append_ellipse_spec(highlights,
                                &n_highlights,
                                lobe_xs[i] + (float)rng_uniform(&rng, -size * 0.03, size * 0.03),
                                lobe_ys[i] - (float)(size * 0.05) +
    ...
    int n_tip_highlight =
        select_unique_indices(&rng, n_tips, rng_int(&rng, 1, 3), selected_tip_idx);
    ...
    batch_stamp_from_specs(
        data, h, w, central_fills, n_central, sh_r, sh_g, sh_b, sh_a, 1.6f, 0.7f);
    batch_stamp_from_specs(data, h, w, shadows, n_shadows, sh_r, sh_g, sh_b, sh_a, 1.8f, 0.8f);
    batch_stamp_from_specs(data, h, w, mids, n_mids, mid_r, mid_g, mid_b, mid_a, 1.5f, 0.7f);
    batch_stamp_from_specs(
        data, h, w, highlights, n_highlights, hi_r, hi_g, hi_b, hi_a, 1.3f, 0.6f);
...
PyObject *brileta_native_sprite_fill_triangle(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(
            args, "Odidiiiii", &canvas_obj, &cx, &top_y, &base_width, &tri_height, &r, &g, &b, &a))
        return NULL;
...
PyObject *brileta_native_sprite_paste_sprite(PyObject *self, PyObject *args) {
    ...
    if (dst_y0 + paste_h > sh)
        paste_h = sh - dst_y0;
    if (dst_x0 + paste_w > sw)
        paste_w = sw - dst_x0;
    ...
    Py_BEGIN_ALLOW_THREADS
        ...
        for (int col = 0; col < paste_w; col++) {
            ...
            float inv_out = 1.0f / out_a;
            dst[0] =
                (uint8_t)clampi((int)((src[0] * sa + dst[0] * da * inv_src) * inv_out), 0, 255);
            dst[1] =
                (uint8_t)clampi((int)((src[1] * sa + dst[1] * da * inv_src) * inv_out), 0, 255);
            dst[2] =
                (uint8_t)clampi((int)((src[2] * sa + dst[2] * da * inv_src) * inv_out), 0, 255);
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                if (PX(data, row, col, w)[3] > 0) {
                    if (row < first_opaque_row)
                        first_opaque_row = row;
                    if (row > last_opaque_row)
                        last_opaque_row = row;
                }
            }
        }
