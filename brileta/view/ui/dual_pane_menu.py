...
class DualPaneMenu(Menu):
    ...
    def _populate_right_pane(self) -> None:
        ...
        items = self.transfer_handler.get_items_from_source()
        countables = self.transfer_handler.get_countables_from_source()
    ...
    def _refresh_cursor_after_right_transfer(
        ...
        if has_selectable_right:
            ...
            if transferred_item is not None:
                new_index = self._find_item_index(transferred_item, PaneId.LEFT)
                if new_index is not None:
                    self.left_cursor = new_index
            self._ensure_valid_cursor(PaneId.LEFT)
        self._update_detail_from_cursor()
    ...
    def _refresh_cursor_after_left_transfer(
        ...
        if has_selectable_left:
            ...
            if transferred_item is not None:
                new_index = self._find_item_index(transferred_item, PaneId.RIGHT)
                if new_index is not None:
