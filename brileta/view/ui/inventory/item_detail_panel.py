    ...
    def generate_detail(
        ...
        *,
        ...
        for label, attack in attacks:
            if attack and WeaponProperty.PREFERRED in attack.properties:  # ty: ignore[unresolved-attribute]
        ...
        if chosen_attack:
            ...
            if label == "Ranged":
                ...
                if WeaponProperty.THROWN in attack.properties:  # ty: ignore[unresolved-attribute]
                    stats = f"Ranged: {attack.damage_dice}"  # ty: ignore[unresolved-attribute]
                else:
                    ammo_str = (
                        f"{attack.current_ammo}/{attack.max_ammo}"  # ty: ignore[unresolved-attribute]
                        if attack.max_ammo  # ty: ignore[unresolved-attribute]
                        else "N/A"
                    )
                    stats = f"Ranged: {attack.damage_dice} [{ammo_str}]"  # ty: ignore[unresolved-attribute]
            else:
                stats = f"{label}: {attack.damage_dice}"  # ty: ignore[unresolved-attribute]

            # Append weapon properties to description (not stats line)
            description.extend(
                WEAPON_PROPERTY_DESCRIPTIONS[prop]
                for prop in attack.properties  # ty: ignore[unresolved-attribute]
