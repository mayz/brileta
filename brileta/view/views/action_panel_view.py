...
from brileta.game.actions.discovery import ActionDiscovery, ActionOption
...
if TYPE_CHECKING:
    ...
    from brileta.game.actions.discovery import TargetType
...
class ActionPanelView(TextView):
    ...
    def __init__(self, controller: Controller) -> None:
        ...
        self._cached_target_type: str | None = None
    ...
    def draw_content(
        ...
        if self._cached_target_name:
            ...
            if self._cached_target_type:
                self.canvas.draw_text(
                    pixel_x=x_padding,
                    pixel_y=y_pixel - ascent,
                    text=self._cached_target_type,
                    color=colors.GREY,
                )
                y_pixel += line_height
    ...
    def _update_cached_data(self) -> None:
        ...
        target_actor, items = self._resolve_hover_target(x, y)
        if items:
            if len(items) == 1:
    ...
    def _resolve_hover_target(
        self, x: int, y: int
    ) -> tuple[Actor | None, list[Item] | None]:
        """Resolve a hover target at a location, preferring actors over items."""
        gw = self.controller.gw
        if not gw.game_map.visible[x, y]:
            return None, None
    ...
    def _populate_actor_target_data(self, target_actor: Actor) -> None:
        ...
        from brileta.game.actions.discovery import classify_target
        ...
        archetype_name = getattr(target_actor, "archetype_name", None)
        self._cached_target_type = (
            archetype_name
            if archetype_name and archetype_name != self._cached_target_name
            else None
        )
    ...
    def _get_default_action_id_for_type(
        ...
        from brileta.game.actions.discovery.types import TargetType as TT
    ...
    def _on_help_click(self) -> None:
        ...
        from brileta.view.ui.commands import OpenMenuUICommand
