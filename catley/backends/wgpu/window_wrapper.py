...
This module integrates Catley's existing GLFW window with the rendercanvas/wgpu
...
class WGPUWindowWrapper(BaseRenderCanvas):
    ...
    existing GLFW window instead of creating a new one. Catley manages the
    window lifecycle and event loop - this wrapper just provides the surface
    for WGPU rendering.
    """

    # No canvas group - Catley manages the event loop
    ...
    def __init__(self, glfw_window: GlfwWindow, vsync: bool = True) -> None:
        ...
        """
        # Disable rendercanvas's internal frame limiting - Catley manages the loop
        super().__init__(
            max_fps=1_000_000,
            vsync=vsync,
    ...
    # no-ops because Catley manages the window and event loop.

    def _rc_gui_poll(self) -> None:
        """Process GUI events. No-op - Catley polls GLFW events."""
        pass

    def _rc_request_draw(self) -> None:
        """Request a draw. No-op - Catley manages the render loop."""
        pass

    def _rc_force_draw(self) -> None:
        """Force a synchronous draw. No-op - Catley manages rendering."""
        pass
    ...
    def _rc_set_logical_size(self, width: float, height: float) -> None:
        """Set logical size. No-op - Catley manages window sizing."""
        pass

    def _rc_set_title(self, title: str) -> None:
        """Set window title. No-op - Catley manages the window title."""
        pass

    def _rc_set_cursor(self, cursor: str) -> None:
        """Set cursor shape. No-op - Catley manages cursors."""
        pass

    def _rc_close(self) -> None:
        """Close the window. No-op - Catley manages window lifecycle."""
        pass

    def _rc_get_closed(self) -> bool:
        """Check if window is closed. Returns False - Catley manages lifecycle."""
        return False
